...
system_name = `/opt/puppetlabs/bin/facter dmi.product.name | awk '{print $2,$3}'`.strip!
...
impi_version = `#{impi_tool} -V`.strip!
...
leds_on    = `#{sudo} #{impi_tool} -I #{impi_interface} -U root sunoem #{led_key} get 2>/dev/null | grep -i SERVICE | grep -cv OFF`.strip!
...
if leds_on == '0'
  ...
  puts `#{sudo} #{impi_tool} -I #{impi_interface} -U root sunoem #{led_key} get 2>/dev/null | grep -i SERVICE | grep -v OFF`.strip!
  puts impi_version
  exit 2
end
