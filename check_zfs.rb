...
def find_mu(mu)
  ...
  [
    ...
    Unit.new('t', 'TB', 1024 * 1024 * 1024 * 1024)
  ].find(not_found) do |u|
    u.mu == mu
  end
end
  ...
  class ScriptOptions
    ...
    def define_pools(parser)
      parser.on('-p', '--pool POOL', Array,
        ...
        'If not given, all pools are checked.'
      ) do |pool|
    ...
    def define_unit(parser)
      parser.on('-u', '--unit UNIT', Unit,
        'The measuring unit for output of sizes - <b|k|m|g|t>'
      ) do |unit|
        self.unit = unit
      end
    end
    ...
    def define_warn(parser)
      parser.on('-w', '--warn USAGE_PERCENT', Float,
        'The warning level of the pool(s) of percent usage.'
      ) do |warn|
        self.warn = warn
      end
    end
    ...
    def define_crit(parser)
      parser.on('-c', '--crit USAGE_PERCENT', Float,
        'The critical level of the pool(s) of percent usage.'
      ) do |crit|
        self.crit = crit
      end
    end
...
if options.pools.empty?
    pools = `#{zpool} list -H | awk '{print $1}'`.strip!.split(/\n/)
    if pools.empty?
...
for pool in pools do
  uhu = `#{zpool} list #{pool} >/dev/null 2>&1`
  unless $?.success?
      puts "UNKNOWN - ZFS Pool '#{pool}' does not exists."
      exit 3
  end

  pool_values = `#{zfs} get -o value -Hp used,available #{pool}`.split("\n")
  pool_state  = `#{zpool} status #{pool} | grep 'state:'  | awk -F':' '{print $2}'`.strip!
  pool_errors = `#{zpool} status #{pool} | grep 'errors:' | awk -F':' '{print $2}'`.strip!
  pool_action = `#{zpool} status #{pool} | grep 'action:' | awk -F':' '{print $2}'`.strip!
  pool_scan   = `#{zpool} status #{pool} | grep 'scan:'   | sed -e 's/[ 	]*scan://'`.strip!
