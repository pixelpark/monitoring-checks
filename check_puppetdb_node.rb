...
OptionParser.new do |opts|
  ...
  opts.on('--ssl', 'Enable https usage.') { |ssl| @options[:ssl] = ssl.nil? ? true : ssl }
...
if last_catalog.nil? || latest_report_hash.nil?
  ...
  hh, mm = mm.divmod(60)
  dd, hh = hh.divmod(24)
  message = "Node will be expire in #{dd} days, #{hh} hours, #{mm} minutes, #{ss} seconds because Puppet hadn't run and a node-ttl of #{@options[:ttl]}s"
  status  = [2, 'CRITICAL']
